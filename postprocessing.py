...
class labelVision:
    ...
    def readLabels(self, unet_output, srcs, fileNames):
        ...
        for mask, src, filename in zip(unet_output, srcs, fileNames):
            ...
            cv2.imwrite(self.Config['results_path'] + "/7_mesh.jpg", mesh)
            cv2.imwrite(self.Config['results_path'] + "/8_unwrapped.jpg", unwrapped)
            return
...
if __name__ == "__main__":
    ...
    unet_output = cv2.imread("mask.jpg", 0)
    src = cv2.imread("./img/train_and_valid/X/134.jpg")
    label.readLabels([unet_output], [src], ['tmp'])